...
class Registry:
    """Gerencia o armazenamento e a criptografia dos secrets no disco local"""
...
def extrair_secrets(env_vars: dict, registry: Registry) -> dict:
    ...
    print(f"leitura do .registry.yml concluida: {len(normais)} variaveis estaticas, {len(secrets_pendentes)} secrets pendentes.")
    
    if not secrets_pendentes:
        return {}
    ...
    return secrets_extraidos

def main():
    parser = argparse.ArgumentParser(description="cofre local para secrets")
    ...
    cmd_run = subparsers.add_parser("run", help="le o yml e mapeia os secrets pendentes")
    ...
    try:
        if args.comando == "init":
            ...
            secrets_extraidos = extrair_secrets(env_vars, registry)
            
            if secrets_extraidos:
                print("secrets a injetar:")
                for chave, valor in secrets_extraidos.items():
                    print(f"\t{chave}={valor}")
            ...
            for chave_env, valor_yml in env_vars.items():
                if str(valor_yml).startswith("secret:"):
                    new_env[chave_env] = secrets_extraidos.get(chave_env, "")
                else:
                    new_env[chave_env] = str(valor_yml)
            
            try:
                subprocess.run(args.comando_alvo, env=new_env)
            except FileNotFoundError:
                print(f"erro: comando '{args.comando_alvo[0]}' nao encontrado no sistema.")
